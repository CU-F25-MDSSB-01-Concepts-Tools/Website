...
# 5. Fit: We use peng_regtree_wf and use fit_resamples() made for folds
peng_regtree_fit <- peng_regtree_wf |> 
...
peng_tune_results <- peng_wf |>
  tune_grid(
    ...
    grid = expand_grid(
        cost_complexity = c(0.01, 0.05, 0.02, 0.01, 0.005),
        min_n = c(2, 10, 20, 40)
    ), 
...
- Many decision trees are built independently (default: 500 trees), each trained on the full dataset
  At each split in each tree:
    - Randomly select `mtry` variables from all available features (e.g., 2 out of 6, rule of thumb: square root of the number of features)
    - Find the best split using only those randomly selected variables. 
    - Grow each tree until terminal nodes contain fewer than `min_n` observations (default, stop when nodes have less than 5 samples)

Make predictions by averaging predictions across all trees (regression) or voting (classification)
The randomness in variable selection at each split ensures trees are different from each other, which reduces overfitting and improves generalization

The key insight: By limiting choices at each split to a random subset of features, we prevent any single strong predictor from dominating all trees, creating a diverse "forest" of models that collectively perform better than any individual tree.
...
## Understand hypothesis testing as a court trial
...
## Automating with `tidymodels`^[Of course, you can also do it in your own way without packages.]
